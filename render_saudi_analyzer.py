...
class RenderSaudiAnalyzer:
    ...
    def generate_realistic_financial_data(self, ticker):
        ...
        # First try to get real data from Tadawul with enhanced anti-bot bypass
        ...
        # Fallback to realistic mock data with real Yahoo Finance prices
        st.info(f"📊 Generating realistic mock financial data with real Yahoo Finance prices for {ticker}...")
    ...
    def get_real_current_price(self, ticker):
        ...
        try:
            ...
            for ticker_variant in ticker_variants:
                try:
                    ...
                    if not hist.empty and len(hist) > 0:
                        ...
                        if current_price > 0:
                            st.success(f"✅ Real price found: {current_price:.2f} SAR ({ticker_variant})")
            ...
            st.info(f"ℹ️ No real price found for {ticker}. Tried: {', '.join(ticker_variants[:3])}...")
            return None
            
        except Exception as e:
            st.warning(f"⚠️ Price lookup error: {e}")
            return None
                    ...
                    with col3:
                        ...
                        if 'Tadawul' in data_source:
                            if current_price and current_price > 0:
                                st.write("**Data Source:** 🏛️ Tadawul (Real) + 📈 Yahoo Finance (Real Prices)")
                            else:
                                st.write("**Data Source:** 🏛️ Tadawul (Real) + 📊 Mock Prices")
                        else:
                            if current_price and current_price > 0:
                                st.write("**Data Source:** 📊 Mock Financials + 📈 Yahoo Finance (Real Prices)")
                            else:
                                st.write("**Data Source:** 📊 Mock Data (Financials + Prices)")
